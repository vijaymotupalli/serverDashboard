...
var dbHandler = {
    ...
    getAppUsers: function (admin) {
        return new Promise(function (resolve, reject) {
            var query = (admin.role == config.institute) ? {institute:admin._id} :{}
            return models.users.find(query,{password:0}).sort({createdAt:-1}).then(function (users, err) {
                if (!err) {
                    resolve(users);
                }
            }).catch(function (error) {
        ...
        return new Promise (function (resolve,reject) {
            return models.codes.findOne({"codes.code":code}).then(function (validCode,err) {
                ...
                validCode.codes.map(function (eachCode) {
                    if(eachCode.code == code && eachCode.usedBy){
               ...
               return models.users.findOne({_id:user}).then(function (user, err) {
                    ...
                    user.institute = validCode.institute
